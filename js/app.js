...
render: { //レンダリングの設定
	options: {
		...
		width: window.parent.screen.width, //canvasのwidth(横幅)
		height: window.parent.screen.height, //canvasのheight(高さ)
...
//座標を取得する
var mX = e.pageX;  //X座標
var mY = e.pageY - window.pageYOffset;  //Y座標

engine.world.gravity.x = (mX - (window.innerWidth/2)) / window.innerWidth;
engine.world.gravity.y = (mY - (window.innerHeight*4/7))/ window.innerHeight;
});
...
engine.world.gravity.y = beta/180;
}, false);
...
var size2 = 20+parseInt(Math.random() * 80);//大きさ
