...
document.onkeydown = function (e){
	...
	if(e.keyCode > 47 && e.keyCode < 91){
		var x = parseInt(Math.random() * window.innerWidth);//幅
		var y = parseInt(Math.random() * window.innerHeight);//高さ
		World.add(engine.world, [
			Bodies.circle(x, y, 40, { //ボールを追加
				
				render: { //ボールのレンダリングの設定
					sprite: { //スプライトの設定
						texture: './images/'+ e.key +'.png', //スプライトに使うテクスチャ画像を指定
						xScale: 0.5,
						yScale: 0.5
					},
...
function create_wall(){
	...
	World.add(engine.world, [
		/*
		//上
		wall[0] = Bodies.rectangle(0, 0, window.innerWidth*2, 40, {
		isStatic: true, //固定する
		...
		*/
		//左
		wall[0] = Bodies.rectangle(0, 0, 40, window.innerHeight*2, {
			isStatic: true, //固定する
		...
		//下
		wall[1] = Bodies.rectangle(0, window.innerHeight, window.innerWidth*2, 40, {
			isStatic: true, //固定する
		...
		//右
		wall[2] = Bodies.rectangle(window.innerWidth, 0, 40, window.innerHeight*2, {
			isStatic: true, //固定する
